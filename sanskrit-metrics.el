...
;;; deletions in LaTeX: comments, plain footnotes (to make this delete footnotes with nesting macros, all macros used in a footnote must be appended either to this list or the latex-keep list), cite and line break
(setq sktm_l-latex-elim '("%.+" "\\\\footnote[A-Z]?{[^{}]+}" "\\\\cite{[^{}]+}" "\\\\\\\\"))

;;; deletions in TEI: comments, plain note elements (to delete notes with nesting elements, all elements used in a note must be appended either to this list or the latex-keep list)
(setq sktm_l-tei-elim '("<!--.+-->" "<note[^>]*>[^<]+</note>"))

;;; delete markup but keep content of LaTeX macro:
(setq sktm_l-latex-keep '("\\\\emph{\\([^{}]+\\)}" "\\\\textbf{\\([^{}]+\\)}" "\\\\textit{\\([^{}]+\\)}" "\\\\textsc{\\([^{}]+\\)}"))
